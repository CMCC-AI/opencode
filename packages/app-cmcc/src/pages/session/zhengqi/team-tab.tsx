  ...
  return (
    <div class="flex size-full min-h-0 flex-col gap-3 px-3 py-3">
      ...
      <div class="grid min-h-0 flex-1 grid-rows-[minmax(0,2fr)_auto_minmax(0,1fr)] overflow-hidden rounded-[8px] border border-[#dfe4ed] bg-white">
        <section aria-label="政企分析 DAG" class="min-h-0 overflow-hidden bg-[#f9fbff] px-3 py-1">
          <div ref={dagContainer} class="relative mx-auto h-full w-full max-w-[860px]">
            ...
            <div
              ...
              style={{ "grid-template-rows": `repeat(${ZHENGQI_DAG_LEVELS.length}, minmax(0, 1fr))` }}
            >
              <For each={ZHENGQI_DAG_LEVELS}>
                {(level) => (
                  <div
                    ...
                    <For each={level}>
                      {(agentId) => {
                        ...
                        return (
                          ...
                          >
                            <AgentAvatar src={zhengqiAvatar(agentId)} name={node()?.name ?? "?"} size="compact" />
...
function DagConnections(props: {
  ...
  const activePaths = createMemo(() => {
    const keys = new Set(
      ZHENGQI_DAG_EDGES.filter(([source, target]) =>
        isZhengqiDagEdgeActive(props.nodes.get(source)?.status, props.nodes.get(target)?.status),
  ...
  onMount(() => {
    ...
    const update = () => {
      ...
      setPaths(
        ZHENGQI_DAG_EDGES.flatMap(([sourceId, targetId]) => {
    ...
    for (const agentId of ZHENGQI_DAG_LEVELS.flat()) {
