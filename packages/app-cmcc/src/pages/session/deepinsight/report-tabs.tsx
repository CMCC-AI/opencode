...
import { DEEPINSIGHT_LEAD_AGENT } from "./config"
import { useDeepInsightWorkbench } from "./workbench-context"

export function DeepInsightFilesTab() {
  ...
  const ownerLabel = (agentId: string) => {
    if (agentId === DEEPINSIGHT_LEAD_AGENT) return "总览"
  ...
  if (context.artifactSource) {
    return (
      <SnapshotFilesTab
        ...
        emptyDescription="该案例快照没有保存深度研究产物文件。"
  ...
  const selected = createMemo(() => files().find((artifact) => artifact.path === state.selectedPath))
  ...
  return (
    <div class="h-full min-h-0 overflow-hidden px-4 py-4">
      <Show
        when={files().length > 0}
        fallback={<ReportEmpty title="暂无文件产出" description="当前深度研究会话确认写入的文件会在这里逐步出现。" />}
      >
        <Show
          when={selected()}
          fallback={
...
export function DeepInsightTextReportTab() {
  const context = useDeepInsightWorkbench()
  return (
    <Show
      ...
      fallback={
        <ReportArtifactPreview
          artifacts={workbenchFiles(context.workbench())}
      ...
      <div class="deeptrading-scrollbar h-full min-h-0 overflow-y-auto bg-[#f7f8fb] px-4 py-4">
        <div class="mx-auto w-full max-w-[860px] rounded-[8px] border border-[#e0e4eb] bg-white px-5 py-5 sm:px-7">
          <Markdown
            ...
            cacheKey={`${context.workbench().rootSessionId}:deepinsight-report:replay`}
            streaming
...
export function DeepInsightVisualReportTab() {
  const context = useDeepInsightWorkbench()
  return (
    <ReportArtifactPreview
      artifacts={workbenchFiles(context.workbench())}
      ...
      empty={<ReportEmpty title="可视化报告尚未生成" description="等待 HTML 格式的报告生成。" />}
    />
  )
}
