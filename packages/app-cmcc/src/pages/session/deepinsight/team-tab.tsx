...
import { DEEPINSIGHT_DAG_EDGES, DEEPINSIGHT_DAG_LEVELS, deepInsightAvatar } from "./config"
import { isDeepInsightDagEdgeActive } from "./data"
import { useDeepInsightWorkbench } from "./workbench-context"

export function DeepInsightTeamTab() {
  const context = useDeepInsightWorkbench()
  ...
  const stats = createMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        label: "报告篇幅",
        value:
          context.workbench().loading || reportLength === undefined ? "--" : formatCaseCharacterCount(reportLength),
  ...
  return (
    <div class="flex size-full min-h-0 flex-col gap-3 px-3 py-3">
      <section aria-label="DeepInsight 统计" class="grid grid-cols-4 gap-1.5">
        <For each={stats()}>
          {(stat) => (
      ...
      <div class="grid min-h-0 flex-1 grid-rows-[minmax(0,2fr)_auto_minmax(0,1fr)] overflow-hidden rounded-[8px] border border-[#dfe4ed] bg-white">
        <section
          aria-label="DeepInsight DAG"
          class="deeptrading-scrollbar min-h-0 overflow-y-auto bg-[#f9fbff] px-3 py-1"
        >
          <div ref={dagContainer} class="relative mx-auto h-full min-h-[432px] w-full max-w-[860px]">
            <DagConnections
            ...
            <div
              ...
              style={{ "grid-template-rows": `repeat(${DEEPINSIGHT_DAG_LEVELS.length}, minmax(0, 1fr))` }}
            >
              <For each={DEEPINSIGHT_DAG_LEVELS}>
                {(level) => (
                  <div
                  ...
                  >
                    <For each={level}>
                      {(agentId) => {
                        ...
                        const upstream = DEEPINSIGHT_DAG_EDGES.filter((edge) => edge[1] === agentId).length
                        return (
                          <button
                            type="button"
                          ...
                          >
                            <AgentAvatar src={deepInsightAvatar(agentId)} name={node()?.name ?? "?"} size="compact" />
...
function DetailEmpty(props: { children: JSX.Element }) {
  return (
...
function DagConnections(props: {
  ...
  const activePaths = createMemo(() => {
    const keys = new Set(
      DEEPINSIGHT_DAG_EDGES.filter(([source, target]) =>
        isDeepInsightDagEdgeActive(props.nodes.get(source)?.status, props.nodes.get(target)?.status),
  ...
  onMount(() => {
    ...
    const update = () => {
      ...
      setPaths(
        DEEPINSIGHT_DAG_EDGES.flatMap(([sourceId, targetId]) => {
    ...
    for (const agentId of DEEPINSIGHT_DAG_LEVELS.flat()) {
