...
import { deepInsightAvatar, deepInsightTeamAvatar } from "./config"
import { useDeepInsightWorkbench } from "./workbench-context"

export function DeepInsightSessionView() {
  ...
  const selected = createMemo(() => {
    ...
    if (context.selectedAgentId() === "overview") {
      return {
        ...
        avatar: deepInsightTeamAvatar(),
      }
    }
    const agent = data.agents.find((item) => item.id === context.selectedAgentId())
    return agent ? { ...agent, avatar: deepInsightAvatar(agent.id) } : undefined
  })
    ...
    <div class="relative flex size-full min-h-0 flex-col overflow-hidden bg-[#f7f8fb] text-[#202636]">
      ...
      >
        <div ref={autoScroll.contentRef} class="mx-auto w-full max-w-[1040px] px-5 pb-32 pt-5 sm:px-8">
          ...
          <Show
            when={selected()}
            fallback={<EmptyPanel title="正在读取深度研究团队" description="正在加载当前深度研究会话。" />}
          >
            {(item) => (
          ...
          <Show when={context.workbench().error}>
            {(error) => <Notice tone="error">深度研究会话加载失败：{error()}</Notice>}
          </Show>
...
function OverviewConversation(props: {
  ...
  return (
    <Show
      ...
      fallback={
        <EmptyPanel
          title={props.status === "waiting" ? "等待专家团开始深度研究分析" : "总览暂未返回正文"}
...
function AgentStrip() {
  ...
  const items = createMemo(() => [
    ...
    ...context.workbench().agents.map((agent) => ({ ...agent, avatar: deepInsightAvatar(agent.id) })),
  ])
