...
import { AI_SCIENCE_DAG_EDGES, AI_SCIENCE_DAG_LEVELS, aiScienceAvatar } from "./config"
import { isAiScienceDagEdgeActive } from "./data"
import { useAiScienceWorkbench } from "./workbench-context"

export function AiScienceTeamTab() {
  ...
  const stats = createMemo(() => {
    ...
    const artifactCount = context.workbench().artifacts.length
    return [
      {
        key: "elapsed",
        label: "思考时间",
      ...
      },
      {
        key: "artifacts",
        label: "科研产物",
        value: context.filesLoading() && artifactCount === 0 ? "--" : `${formatNumber(artifactCount)} 个文件`,
  ...
  return (
    <div class="flex size-full min-h-0 flex-col gap-3 px-3 py-3">
      <section aria-label="AI for Science 统计" class="grid grid-cols-4 gap-1.5">
        <For each={stats()}>
          {(stat) => (
      ...
      <div class="grid min-h-0 flex-1 grid-rows-[minmax(0,2fr)_auto_minmax(0,1fr)] overflow-hidden rounded-[8px] border border-[#dfe4ed] bg-white">
        <section aria-label="AI for Science DAG" class="min-h-0 overflow-hidden bg-[#f9fbff] px-3 py-1">
          <div ref={dagContainer} class="relative mx-auto h-full w-full max-w-[900px]">
            <DagConnections
            ...
            <div
              ...
              style={{ "grid-template-rows": `repeat(${AI_SCIENCE_DAG_LEVELS.length}, minmax(0, 1fr))` }}
            >
              <For each={AI_SCIENCE_DAG_LEVELS}>
                {(level) => (
                  <div
                    ...
                    <For each={level}>
                      {(agentId) => {
                        ...
                        return (
                          ...
                          >
                            <AgentAvatar src={aiScienceAvatar(agentId)} name={node()?.name ?? "?"} size="compact" />
          ...
          <div class="deeptrading-scrollbar min-h-0 flex-1 overflow-y-auto px-3 py-3">
            <Switch>
              <Match when={context.selectedAgentId() === "overview"}>
                <DetailEmpty>请选择上方专家查看执行记录</DetailEmpty>
              </Match>
...
const DAG_VIEWBOX_HEIGHT = 1_100
...
function DagConnections(props: {
  ...
  const activePaths = createMemo(() => {
    const keys = new Set(
      AI_SCIENCE_DAG_EDGES.filter(([source, target]) =>
        isAiScienceDagEdgeActive(props.nodes.get(source)?.status, props.nodes.get(target)?.status),
  ...
  onMount(() => {
    ...
    const update = () => {
      ...
      setPaths(
        AI_SCIENCE_DAG_EDGES.flatMap(([sourceId, targetId]) => {
    ...
    for (const agentId of AI_SCIENCE_DAG_LEVELS.flat()) {
...
function edgePath(source: HTMLButtonElement, target: HTMLButtonElement, container: DOMRect) {
  ...
  if (Math.abs(sourceCenterY - targetCenterY) < Math.max(sourceRect.height, targetRect.height)) {
    const forward = targetRect.left > sourceRect.left
    const sourcePoint = toPoint(forward ? sourceRect.right : sourceRect.left, sourceCenterY, container)
    const targetPoint = toPoint(forward ? targetRect.left : targetRect.right, targetCenterY, container)
