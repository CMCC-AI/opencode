...
export const { use: useDockApi, provider: DockApiProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const clear = () => {
      ...
      removePersisted(Persist.window("tabs"), platform)
      removePersisted(Persist.window("tabs.recent"), platform)
      removePersisted(Persist.window("tabs.info"), platform)
      removePersisted(Persist.window("tabs.closed"), platform)
      setState({
...
export function useDockApiSession(): ProductSessionAdapter {
  const dockapi = useDockApi()
  const binding = (sessionID: string) => dockapi.sessions.findByOpenCodeId(sessionID)
  const directory = () => dockapi.workspace?.directoryPath

  return {
    directory,
    eventDirectory: directory,
    managesDirectory: (value) => value === directory(),
    owns: (sessionID) => !!binding(sessionID),
    async create(input) {
      if (!input.query.trim()) throw new DockApiError("新会话需要输入文本问题")
      const session = await dockapi.sessions.create({ query: input.query, title: input.title })
      const created = asOpenCodeSession(session.openCodeSession)
      if (!created) throw new DockApiError("业务会话未返回有效的 OpenCode session")
      return { ...created, title: session.title, directory: session.directoryPath }
    },
    async list(input) {
      if (input.directory !== directory()) return
      const sessions = (await dockapi.sessions.list()).filter((item) => item.directoryPath === input.directory)
      const settled = await Promise.allSettled(
        sessions.map(async (item) => ({
          ...(await input.load(item.openCodeSessionId, item.directoryPath)),
          title: item.title,
    ...
    async resolve(input) {
      const item = binding(input.sessionID)
      if (!item) return
      return {
        ...(await input.load(item.openCodeSessionId, item.directoryPath)),
        title: item.title,
    ...
    rename: (sessionID, title) => dockapi.sessions.updateTitle(sessionID, title),
    remove: (sessionID) => dockapi.sessions.remove(sessionID),
    canArchive: (sessionID) => !binding(sessionID),
  }
}
