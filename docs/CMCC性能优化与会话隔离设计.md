...
通用 App 部署环境优先使用服务端 `path.directory`，也就是部署脚本配置的 `WorkingDirectory`（默认 `/srv/opencode/workspaces`）；本地开发在没有该路径时回退到 `~/Documents/DeepInsight`。DockAPI 登录版以业务后端返回的 `u-{userId}` 作为该用户的稳定 runtime，每个会话使用 `u-{userId}/runs/{timestamp-uuid}`。实现位于 `packages/app-cmcc/src/utils/cmcc-workspace.ts` 和 `packages/app-cmcc/src/context/dockapi.tsx`。
...
DockAPI 登录版由业务后端在用户稳定 runtime 中创建 OpenCode session，并在同一次请求中保存 MySQL 会话绑定；前端不直接创建后再补绑，也不再通过 Redis 提前创建临时 OpenCode session。
...
默认列表在 `packages/app-cmcc/src/context/model-defaults.ts` 中集中定义：

- 首次进入时优先显示和选择默认模型；
- “管理模型”中由用户开启的其他已连接 Provider 模型也会出现在选择器中；
- session、Agent、全局配置和最近使用模型只要仍属于已连接 Provider 即可继续使用；
- 首页默认回退优先从当前已连接 Provider 的默认模型中选择；
- 当前模型被隐藏后会暂时保留在选择器中，切换后按新的可见性设置展示。

这样既保留产品默认推荐，也让“管理模型”的可见性开关真正控制外层模型选择器。
